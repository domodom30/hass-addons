...
│       └── static/                      # Built Vue SPA (copied in at image build)
├── frontend/                            # Vue 3 + Vite source (built to static/)
│   ├── package.json / vite.config.js    # Build tooling
│   └── src/                             # App.vue, views/, components/, stores/, composables/
├── docker/                              # Container build
│   ├── Dockerfile                       # Alpine 3.20 image (+ node build stage)
...
### Frontend (`frontend/` → built into `web/static/`)

A single-page application built with **Vue 3 (SFC) + Vite**, with **Pinia** for
state and **Bootstrap 5 + Font Awesome vendored** into the bundle (no CDN, so it
works on offline HA installs). Source lives in `frontend/`; Vite builds it to
`frontend/dist/` which the Docker image copies into `src/.../web/static/`.

**State (Pinia stores):** `devices` (raw list + stale-fade-out merge + stable
sort + sink matching), `events` (MPRIS/AVRCP log with volume de-dup), `logs`
(ring buffer + filtering), `connection` (scan countdown), `settings`, `info`
(feature flags from `/api/info`), `ui` (toasts, banners, active view) and
`modals` (centralized modal visibility so nested components avoid prop drilling).

**Three views:**
- **Devices** — Card grid (`DeviceCard`) with status, capability badges, RSSI, and action buttons
- **Events** — Real-time MPRIS/AVRCP event log (`EventEntry`)
- **Logs** — Live application log with level/text filtering and a "live" freeze

**Bootstrap JS is not used.** Modals (`BaseModal` via `Teleport`), toasts and
dropdowns are reimplemented as state-driven Vue components.

**WebSocket reconnection** (`composables/useWebSocket.js`): on disconnect, shows
a banner with elapsed time and reconnects with exponential backoff (1.5×, capped
at 30 s). Each message is routed to the relevant store; sink changes need no
manual re-render because cards read sinks reactively.

**Build & tests:** `cd frontend && npm ci && npm run build` (or `npm run test`
for Vitest, `npm run lint` for ESLint). CI runs lint + unit tests in a
`frontend-checks` job that gates the image builds.
...
The UI started as vanilla JS, but state grew dispersed (multiple device caches,
ad-hoc globals) and rendering was 150+ line HTML-string builders. Vue's
reactivity removes the manual cache/DOM synchronization, and SFC components make
the device card, badges, events and modals reusable and testable. Vite gives
content-hashed assets (no manual cache-busting) and a fast build. Bootstrap and
Font Awesome are vendored into the bundle so nothing is fetched from a CDN at
runtime — important for offline HA installs. The build step is confined to a
Docker stage pinned to `--platform=$BUILDPLATFORM`, so it runs natively on the
CI runner and never under QEMU.

### Why `/res/` instead of `/static/`?

HA's frontend service worker intercepts any URL containing `/static/` with
`CacheFirst` strategy and `ignoreSearch: true`, so a cached file is never
re-requested. Vite is configured with `base: "./"` and `assetsDir: "res"`, so it
emits relative `./res/<hash>.js` URLs (relative is required because the HA
ingress prefix is dynamic). The aiohttp server exposes the built assets under a
`/res/` static mount, avoiding the service worker's URL pattern entirely. Because
filenames are content-hashed, assets are served `immutable` while `index.html`
stays `no-cache`.
